...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(traquinas, code error - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(traquinas, server error. - "+str(e.reason)+",10000,"+icon+")")
def SKindex():
    addDir('[B][COLOR lime][ Favoritos ][/COLOR][/B]','[B][COLOR lime][ Favoritos ][/COLOR][/B]',4,'special://home/addons/plugin.video.traquinas/resources/Favoritos.png',FANART,'[COLOR white][B]Traquinas[COLOR lime].[COLOR white]HD[/B][/COLOR]','','','')
    getData(Base,FANART)
...
def getSources():
        try:
            if os.path.exists(favorites) == True:
                addDir('[B][COLOR lime]| Favorites[/COLOR][/B]','[B][COLOR lime]| Favorites[/COLOR][/B]',4,'special://home/addons/plugin.video.traquinas/resources/Favoritos.png' , os.path.join(home, 'fanart.gif'),FANART,'','','','')
...
def getSearchData(url,icon, data=None):
    ...
    keyboard.setHeading("[COLOR white][B]Traquinas[COLOR lime].[COLOR white]HD Search[/B][/COLOR]")
    keyboard.setDefault('')
    keyboard.doModal()
    if keyboard.isConfirmed():
        ...
        xxbmcgui.Dialog().ok('[COLOR white][B]Traquinas[COLOR lime].[COLOR white]HD[/B][/COLOR]', '[COLOR white][B]Blank Searches are not allowed.[/B][/COLOR]')
        quit()
    fanart=''
    dontLink=False
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(traquinas,Unable to play empty Url,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(traquinas,Unable to play check proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(traquinas,Urlresolver doesn´t support this domain. - ,5000)")
        resolver=url
    return resolver
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('[B][COLOR blue]Remove Traquinas Favorite[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('[B][COLOR lime]Add to Traquinas Favorites[/COLOR][/B]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('[B][COLOR blue]Remove Traquinas Favourite[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('[B][COLOR lime]Add to Traquinas Favorites[/COLOR][/B]','XBMC.RunPlugin(%s)' %fav_params))                                                                                
            liz.addContextMenuItems(contextMenu)
        try:
...
params=get_params()
Base = 'aHR0cDovL2ZyZW5jaGRqLmF0c3BhY2UudHYvdHJhcXVpbmFzLnBocD91c2VyPQ=='.decode('base64')+USER+'JnBhc3M9'.decode('base64')+PASS
url=None
name=None
mode=None
playlist=None
...
#            ln+='</item>'
        ...
        if not (regexs and 'notplayable' in regexs and not url):        
            if url:
                ...
                xbmc.executebuiltin("XBMC.Notification(traquinas,fail to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(traquinas,Install [COLOR yellow]  Youtube-dl[/COLOR] module ,10000,"")")
